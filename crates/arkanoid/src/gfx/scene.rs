...
use engine::math::{game_to_world, Vec3};
...
pub fn collect_opaque(game: &Game) -> Vec<CrystalInstance> {
    ...
    let rail_hx = WORLD_W * 0.5 - WALL - 0.6;
    opaque.push(CrystalInstance::new(
        gw(WORLD_W * 0.5, WALL * 0.5, 16.0),
        0.05,
        Vec3::new(rail_hx, WALL * 0.5, 16.0),
        0.0,
        z0,
        wall_c,
    ));
    ...
    let paddle_c = if game.laser_t > 0.0 {
        [1.0, 0.78, 0.28, 1.0]
    } else if game.expand_t > 0.0 {
        [0.45, 0.95, 1.0, 1.0]
    ...
    if game.laser_t > 0.0 {
        let barrel = pw * 0.5 - 14.0;
        for side in [-1.0, 1.0] {
            opaque.push(CrystalInstance::new(
                gw(game.paddle_x + side * barrel, PADDLE_Y - 7.0, 14.0),
                1.1,
                Vec3::new(3.4, 6.0, 3.4),
                0.0,
                z0,
                [1.0, 0.86, 0.42, 1.0],
            ));
